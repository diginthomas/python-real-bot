...
class Insert:
    def __init__(self,listings_price,listings_description,listings_url,listings_image,listings_date):
        ...
        self.listings_date = listings_date
    def insert(self):
        # zip function will make all the list the same size
        # getting the values of price, url and image 
        for i, j, k, z, a in zip(self.listings_price,self.listings_description,self.listings_url,self.listings_image,self.listings_date):
            if (i.text == 'Please Contact'):
                continue
            con.execute("""INSERT INTO SCRAP(PRICE,DESCRIPTION,URL,IMAGES,DATE) VALUES (?, ?, ?, ?, ?);""", (float(i.text.replace('$','').replace(',','')), j.text, k.get('href'), z.get('src'),a.text))
        con.commit()
        con.close()
